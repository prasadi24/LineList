...
{
    public class EpProjectInsulationRowController : Controller
    {
        ...
        public EpProjectInsulationRowController(IEpProjectInsulationDefaultRowService insulationDefaultRowService,
        ...
        [HttpPost]
        public async Task<JsonResult> Update(EpProjectInsulationDefaultRowEditDto model)
        {
            ...
            var insulationDefault = _mapper.Map<EpProjectInsulationDefaultRow>(model);
        ...
        public async Task<IActionResult> UpdateRow(Guid id)
        {
            ...
            var insulationRowDto = _mapper.Map<EpProjectInsulationDefaultRowEditDto>(insulationRow);
        ...
        {
            EpProjectInsulationDefaultRow row = await _insulationDefaultRowService.GetById(id);
            if (row == null)
        ...
        {
            EpProjectInsulationDefaultRow row = new EpProjectInsulationDefaultRow();
            ...
            if (isExist)
                return Json(new { success = false, ErrorMessage = "The selected Size NPS setting already exists as a row in this table." });
            row.SizeNpsId = sizeNpsId;
            row.EpProjectInsulationDefaultId = insulationDefaultId;
